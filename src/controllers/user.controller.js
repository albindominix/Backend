...
console.log(req.query,"req.query")
  let user = await userService.getUserById(requestedUserId);
   if(!user){
  ...
  if(req.query.q === "address"){
    let userAddress = await userService.getUserAddressById(req.user.id)
    res.status(httpStatus.OK).send({address: userAddress.address});
  }
  else{
